...
**[FastMD 1.1.1 — последний релиз](https://github.com/beanbo/FastMD/releases/latest):**
...
2. Или `FastMD-1.1.1-win-x64.zip` — распакуйте куда угодно и запустите `FastMD.exe`; чтобы `.md` открывались двойным кликом, выберите в контекстном меню «Открывать .md в FastMD…» и подтвердите выбор в «Параметрах».
...
- **Работа с текстом:**
  ...
  - флажки в списках задач отмечаются кликом прямо в файле (`- [ ]` ↔ `- [x]`): меняется только символ между скобками и только если файл на диске в точности тот, что на экране. Это единственный случай, когда FastMD пишет в документ.
