...
| The final gate: a phantom row stayed at its old offset when its block changed length other than by the core's typing (a popup's text, an inline formula emptied, the code toggle and language, typing in a big document); the next letter typed into the row went there - into the text, the code, the closing fence | §6.7 | the row stays beside its block through every change: the operations that never make a phantom carry it with the caret in it too (`Carry(r, true)`), and the glue's own splices (a popup, a big document's typing) carry it as well |
| The final gate: a recovery file whose save went through stayed when a scanner (an antivirus, the indexer) had it open without letting others delete it; once the document changed, it read as an interrupted save - the strip, every save refused (`test_tasks`' rare failure) | §10.5 | a recovery file done with that cannot be deleted goes on a list: deleted again at the next save, search and flush point, and passed over by the search meanwhile |
