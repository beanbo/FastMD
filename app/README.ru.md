...
| `src/highlight.cpp` | подсветка внутри блоков кода: однопроходный разборщик без грамматики (комментарии, строки, числа, ключевые слова, типы с большой буквы, вызовы, `$переменные`) в цветах GitHub; у HTML, XML, SVG и XAML свой маленький разборщик: он красит теги, атрибуты, значения, комментарии и сущности, текст между тегами оставляет обычным, а `<script>` / `<style>` отдаёт правилам JavaScript и CSS; свой разборщик и у YAML (ключи, значения строками, если это не числа, булевы или null, комментарии, якоря, теги, блоки `|` / `>`); в shell, batch, Dockerfile, Makefile, CMake, INI/TOML, conf/.env и TeX слово с большой буквы не считается типом |
