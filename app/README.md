...
- **Настройки** (Ctrl+, или контекстное меню) применяются сразу и доходят до других открытых окон:
...
- Нет сносок (нужен md4c master), печати и доступности для экранных дикторов (UIA).
...
| `src/view.cpp` | колонки и их пресеты, вынос широких блоков и их прокрутка вбок, виртуализированная вёрстка, отрисовка, hit-testing, выделение, фокус ссылок |
...
| `src/loader.cpp` | загрузка документа (стартовый поток параллельно с созданием окна; открытие, перезагрузка, история), фоновые замеры высот, WIC, наблюдение за файлом, позиции чтения |
...
| `src/settings_ui.cpp` | окно настроек, нарисованное тем же движком |
...
| `src/canvas_gdi.cpp` | CPU-рендер: DirectWrite → DIB, SDF-растеризатор фигур, послойные COLR-эмодзи, попиксельное отсечение текста |
...
Всё новое этапа 1 стоит вне критического пути первого кадра. Поле поиска, чтение `positions.bin`, окно настроек и поиск редакторов создаются после первого кадра или по действию. Исключение — стартовый экран без документа: его список читается вместе с запуском.
...
- **UI-тест** запускает настоящие окна и проверяет 59 вещей: выделение, копирование, поиск, ссылки, зум, тему, историю, живую перезагрузку, положение окна и каждую задачу этапа 1 по её критерию готовности из плана. Состояние читается через `WM_APP_QUERY`. Тест работает в своём профиле (`FASTMD_REGKEY`, `FASTMD_DATA`) и не трогает ваши настройки. Скриншоты сохраняются в `app/tests/out/`.
  ```
