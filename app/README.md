...
- **UI-тест** запускает настоящие окна и проверяет 62 вещи: выделение, копирование, поиск, ссылки, зум, тему, историю, живую перезагрузку, положение окна, значок настроек и каждую задачу этапа 1 по её критерию готовности из плана. Состояние читается через `WM_APP_QUERY`. Тест работает в своём профиле (`FASTMD_REGKEY`, `FASTMD_DATA`) и не трогает ваши настройки. Скриншоты сохраняются в `app/tests/out/`.
  ```
